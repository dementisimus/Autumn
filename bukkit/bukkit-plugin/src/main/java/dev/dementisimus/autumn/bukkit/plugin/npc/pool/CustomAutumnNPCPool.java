...
package dev.dementisimus.autumn.bukkit.plugin.npc.pool;
...
public class CustomAutumnNPCPool implements AutumnNPCPool, Listener {
    ...
    private static final AtomicInteger ENTITY_COUNT = Objects.requireNonNull(AutumnReflectionMethod.getterStatic(net.minecraft.world.entity.Entity.class, "d", AtomicInteger.class)).get();
    ...
    @Getter
    private final Map<String, AutumnDoubleCallback<Player, AutumnBukkitTranslation>> translationTasks = new HashMap<>();
    ...
    public CustomAutumnNPCPool(BukkitAutumn bukkitAutumn, Plugin plugin) {
        ...
        ProtocolLibrary.getProtocolManager().addPacketListener(new PacketAdapter(plugin, PacketType.Play.Client.USE_ENTITY) {
            ...
            public void onPacketReceiving(PacketEvent event) {
                ...
                if (npc != null) {
                    List<AutumnDoubleCallback<Player, AutumnNPC.AutumnNPCInteractionAction>> interactions = new ArrayList<>();
                    ...
                    Bukkit.getScheduler().runTask(CustomAutumnNPCPool.this.plugin, () -> {
                        for (AutumnDoubleCallback<Player, AutumnNPC.AutumnNPCInteractionAction> interactionRequest : interactions) {
    ...
    public AutumnNPC register(AutumnNPC npc) {
        ...
        if (customAutumnNPC.getSkinValue() == null || customAutumnNPC.getSkinSignature() == null) {
            this.bukkitAutumn.logging().error("No skin value and/or signature defined for npc!");
            return null;
        }
    ...
    @Override
    public void onInteraction(String id, AutumnDoubleCallback<Player, AutumnNPC.AutumnNPCInteractionAction> interaction) {
        this.requestedInteractions.add(new RequestedInteraction(id, interaction));
    }

    @Override
    public void onTranslation(String id, AutumnDoubleCallback<Player, AutumnBukkitTranslation> context) {
        ...
        for (AutumnNPC npc : this.npcsByEntityId.values()) {
            if (npc.canSee(player)) {
                if (((CustomAutumnNPC) npc).isPushBack() && ((CustomAutumnNPC) npc).getBoundingBox().overlaps(((CustomAutumnNPC) npc).boundingBoxOf(event.getTo()))) {
                    org.bukkit.util.Vector vector = event.getFrom().toVector().subtract(((CustomAutumnNPC) npc).getLocation().toVector()).normalize();
        ...
        if (gson == null) {
            ...
            gsonBuilder.registerTypeAdapter(ItemStack.class, new TypeAdapter<ItemStack>() {
                ...
                public JsonElement serialize(ItemStack itemStack, Type type, JsonSerializationContext jsonSerializationContext) {
                    return new JsonPrimitive(AutumnItemStackSerializerUtil.itemToString(itemStack));
                }

                @Override
                public ItemStack deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
                    return AutumnItemStackSerializerUtil.stringToItem(jsonElement.getAsString());
                }
            });
    ...
    private record RequestedInteraction(String id,
                                        AutumnDoubleCallback<Player, AutumnNPC.AutumnNPCInteractionAction> interaction) {
    }
}
